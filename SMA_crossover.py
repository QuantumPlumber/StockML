...
def instrument_price(sell_price, buy_price, base_price=3.5, delta=.5):
    delta_price = (sell_price - buy_price) * delta
    return delta_price + base_price


def bollinger_strat(time, sma, sma_short, bollinger_down, bollinger_up, candle, candle_down, candle_up):
    stop_loss = .8
    call_thresholds = {'d_buy': 0.}
    put_thresholds = {'d_buy': 0.}
    ...
    for i in np.arange(sma.shape[0])[start_offset:]:
        ...
        if (gm_time[3] - 4 > 9) and (gm_time[3] - 4 < 16):
            ...
            pre_threshold = (candle[i - 1] - sma[i - 1])/
            threshold = candle[i] - sma[i]
            trigger = (pre_threshold < call_thresholds['d_buy']) and (threshold > call_thresholds['d_buy'])

            if trigger and not open_call_position:  # open call options
                call_value = instrument_price(sell_price=candle[i], buy_price=candle[i])
                call_buy_locs.append(i)
                call_buy_price.append(candle[i])
                open_call_position = True

            if open_call_position:
                current_call_value = instrument_price(sell_price=candle[i], buy_price=call_buy_price[-1])
                if current_call_value < stop_loss * call_value:  # close call options stoploss
                    call_sell_locs.append(i)
                    call_sell_price.append(candle[i])
                    open_call_position = False
                print(current_call_value/call_value)

                if current_call_value > call_value:
                    call_value = current_call_value

            '''
            trigger = (pre_threshold > put_thresholds['d_buy']) and (threshold < put_thresholds['d_buy'])

            if trigger and not open_put_position:  # open put options
                put_value = instrument_price(sell_price=candle[i], buy_price=candle[i])
                put_buy_locs.append(i)
                put_buy_price.append(candle[i])
                open_put_position = True

            if open_put_position:
                current_put_value = instrument_price(sell_price=candle[i], buy_price=put_buy_price[-1])
                if current_put_value > stop_loss * put_value and open_put_position:  # close call options stoploss
                    put_sell_locs.append(i)
                    put_sell_price.append(candle[i])
                    open_put_position = False
                #print(current_put_value)
