...
def SMA_strat(time, sma, sma_d, sma_dd, bollinger_down, bollinger_up, candle, candle_down, candle_up):
    ...
    call_thresholds = {'d_buy': .005, 'dd_buy': 0.05, 'd_sell': 0.005, 'dd_sell': -0.02}
    put_thresholds = {'d_buy': 0.005, 'dd_buy': -0.03, 'd_sell': 0.005, 'dd_sell': 0.02}
    ...
    for i in np.arange(sma.shape[0])[start_offset:]:
        ...
        if (gm_time[3] - 4 > 9) and (gm_time[3] - 4 < 16):
            ...
            if np.absolute(sma_d[i]) < call_thresholds['d_buy'] and sma_dd[i] > 0 and not open_call_position:  # open call options
                max_call_price = candle[i]
                call_buy_locs.append(i)
                call_buy_price.append(candle[i])
                open_call_position = True

            if np.absolute(sma_d[i]) < call_thresholds['d_sell'] and sma_dd[i] < 0 and open_call_position:  # close call options if the sma falls below threshold
                call_sell_locs.append(i)
                call_sell_price.append(candle[i])
                open_call_position = False


            if max_call_price < candle[i]:
                max_call_price = candle[i]
            elif max_call_price open_call_position:
                call_sell_locs.append(i)
                call_sell_price.append(candle[i])
                open_call_position = False


            if np.absolute(sma_d[i]) < call_thresholds['d_buy'] and sma_dd[i] < 0 and not open_put_position:  # open put options
                max_put_price = candle[i]
                put_buy_locs.append(i)
                put_buy_price.append(candle[i])
                open_put_position = True

            if np.absolute(sma_d[i]) < call_thresholds['d_buy'] and sma_dd[i] > 0 and open_put_position:  # close put options
                put_sell_locs.append(i)
                put_sell_price.append(candle[i])
                open_put_position = False

            if max_put_price > candle[i]:
                max_put_price = candle[i]
            elif open_put_position:
                put_sell_locs.append(i)
                put_sell_price.append(candle[i])
                open_put_position = False
...
if __name__ == "__main__":
    ...
    plt.hist(np.absolute(sma_d), bins=100)
