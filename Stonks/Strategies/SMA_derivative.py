...
def SMA_strat(time, sma, bollinger_down, bollinger_up, candle, candle_down, candle_up):
    crossover_threshold = .0
    stop_loss_fraction = .02

    buy_call_threshold = .0
    sell_call_thershold = -.005
    buy_put_threshold = .0
    sell_put_thershold = .005

    call_thresholds = {'d_buy': 0, 'dd_buy': 0.05, 'd_sell': -0.01, 'dd_sell': -0.02}
    put_thresholds = {'d_buy': 0, 'dd_buy': -0.03, 'd_sell': 0.01, 'dd_sell': 0.02}
    ...
    for i in np.arange(sma.shape[0])[start_offset:]:
        ...
        if (gm_time[3] - 4 > 9) and (gm_time[3] - 4 < 16):
            ...
            # delta = (sma[i - look_back:i][1:] - sma[i - look_back:i][:-1]) / (
            #        bollinger_up[i - look_back:i][1:] - bollinger_down[i - look_back:i][1:])
            delta = (sma[i - look_back:i][1:] - sma[i - look_back:i][:-1])
            delta = np.sum(delta) / delta.shape[0]

            deltadelta = (sma[i - look_back] - 2 * sma[i - look_back // 2] + sma[i]) / 2

            if deltadelta > call_thresholds['dd_buy'] and not open_call_position:  # open call options
                call_buy_locs.append(i)
                call_buy_price.append(candle[i])
                open_call_position = True

            if deltadelta < call_thresholds['dd_sell'] and open_call_position:  # close call options if the sma falls below threshold
                call_sell_locs.append(i)
                call_sell_price.append(candle[i])
                open_call_position = False

            if deltadelta < put_thresholds['dd_buy'] and not open_put_position:  # open put options
                put_buy_locs.append(i)
                put_buy_price.append(candle[i])
                open_put_position = True

            if deltadelta > put_thresholds['dd_sell'] and open_put_position:  # close put options
