    ...
    for i in np.arange(sma.shape[0]):
        ...
        if (gm_time[3] - 4 > 9) and (gm_time[3] - 4 < 16):
            ...
            if open_put_position:
                put_price = instrument_price(candle[i], put_buy_price[-1], base_price=3,
                                             delta=.5)
                if put_price >= max_put_price:
                    max_put_price = put_price
                # print(put_price)
                if put_price < put_thresholds['stop_loss'] * max_put_price or \
                        put_price > put_thresholds['profit'] * put_buy_option_price[-1]:  # close put options
                    # print('#############################################')
